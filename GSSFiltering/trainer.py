...
class Trainer():
    ...
    def train_batch(self):
        ...
        elif self.mode == 1: # Split KalmanNet
    ...
    def train_batch_alternative(self):
        ...
        for i in range(self.batch_size):
            ...
            # cov_hat[i] = torch.diagonal(self.dnn.cov_post_byK_optA_history[:,:,-self.seq_len:]).T # Extract the diagonal of the predicted covariance
            cov_hat[i] = torch.diagonal(self.dnn.cov_history[:,:,-self.seq_len:]).T # Extract the diagonal of the predicted covariance
            self.dnn.reset(clean_history=False)

        if (isinstance(self.dnn, Split_KalmanNet_Filter)):
            # Loss Functions: GNLL / Empirical Averaging

            # loss = gaussian_nll(batch_x[:, :, 0:], x_hat[:, :, 0:], cov_hat[:, :, 0:])

            loss = empirical_averaging(batch_x[:, :, 1:], x_hat[:, :, 1:], x_hat[:, :, 1:], beta=self.beta*(self.train_count / int(config['Train']['train_iter'])))  # beta ranges from 0 to 500/1000=0.5

            # BETA = self.beta * (self.train_count / int(config['Train']['train_iter']))
            # loss = \
            #     + (1-BETA) * torch.mean( torch.square( batch_x[:, :, 1:]-x_hat[:, :, 1:] ) ) \
            #     + (BETA) * torch.sum(torch.abs((batch_x[:, :, 1:]-x_hat[:, :, 1:])**2 - cov_hat[:, :, 1:] ))
    ...
    def train_batch_joint(self):
        ...
        if self.isBNN:
            ...
            # loss = self.loss_fn(batch_x[:,:,1:], x_hat[:,:,1:])
            # loss = gaussian_nll(batch_x[:, :, 1:], x_hat[:, :, 1:], cov_hat[:, :, 1:])
